...
import JournalPageSkeleton from "@/app/components/skeletons/JournalPageSkeleton";
...
export default function JournalPage() {
  ...
  if (isPending) return <JournalPageSkeleton />;

  return (
    <div className="min-h-screen bg-[#0a0a0f] grid lg:grid-cols-[300px_1fr] lg:px-20 md:px-8 px-4 font-sans text-white">
      {/* Sidebar */}
      <div className="border-r border-white/[0.07] lg:px-5 py-10 flex flex-col gap-3">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl font-light tracking-tight">Journal</h1>
          <button
            onClick={handleNew}
            className="w-7 h-7 bg-indigo-500 cursor-pointer rounded-lg text-white text-lg flex items-center justify-center transition-colors"
          >
            +
          </button>
        </div>
        ...
        {journals && journals.length == 0 && (
          <p className="text-zinc-500 text-sm">No Journals Added Yet</p>
        )}
      </div>
      ...
      <form
        ...
        className="lg:px-16 py-12 flex flex-col gap-8 max-w-3xl"
      >
          ...
          <div className="flex flex-wrap gap-2">
            {PROMPTS.map((prompt) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setSelectedPrompt(prompt === selectedPrompt ? null : prompt)
                }
        ...
        <textarea
          ...
          onChange={(e) =>
            setWordCount(
              e.target.value.trim().split(" ").filter(Boolean).length,
            )
          }
          required
